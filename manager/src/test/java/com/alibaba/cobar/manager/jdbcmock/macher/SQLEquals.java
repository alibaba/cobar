...
public class SQLEquals implements IArgumentMatcher {
    ...
    public boolean matches(Object actualSQL) {
        if (actualSQL == null && expectedSQL == null)
            ...
            if((((String) actualSQL).toLowerCase()).startsWith("switch @@datasource"))
                return true;
            else if((((String) actualSQL).toLowerCase()).startsWith("kill @@connection"))
                return true;
            else if((((String) actualSQL).toLowerCase()).startsWith("stop @@heartbeat"))
                return true;
            else 
